...
namespace BlueBell
{
		...
		, m_pInputLayout(nullptr)
	{
		...
		D3D11_INPUT_ELEMENT_DESC* pElementDesc = BlueBerry()->AllocateArray<D3D11_INPUT_ELEMENT_DESC>(variables.GetSize());
		...
		HRESULT hr = pDevice->CreateInputLayout(pElementDesc, variables.GetSize(), pVertexBlob->GetBufferPointer(), pVertexBlob->GetBufferSize(), &m_pInputLayout);
